  ...
  useEffect(() => {
    ...
    const fetchProfileData = async () => {
      try {
        ...
        const roadmapSnap = await getDoc(
          doc(db, "users", user.uid, "roadmap", "current"),
        );
        const platformsSnap = await getDoc(
          doc(db, "users", user.uid, "fingerprint", "platform_profiles"),
        );
          ...
          try {
            ...
            if (username) {
              ...
              if (res.ok) {
                ...
                githubReposText = repos
                  .map(
                    (r: any) =>
                      `- ${r.name} (${r.language || "N/A"}): ${r.description || "No description"}`,
                  )
                  .join("\\n");
              }
            }
...
`;
  ...
  const sendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const finalModelText = await processAgentActions(
  ...
  const activeMessages = messages.filter(
    (m) => (m.sessionId || "legacy") === (activeSessionId || "legacy"),
  );
          ...
          >
            ...
            <div className="p-4 bg-slate-900 flex justify-between items-center text-white">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-bold text-sm font-display tracking-tight">
                    AI Mentor
                  </h3>
            ...
            >
              {activeMessages.length === 0 && (
                <div className="text-center py-10 opacity-60">
                  ...
                  <p className="text-sm">
                    Hi! Paste your GitHub profile or resume link, and ask me to
                    add your skills automatically!
                  </p>
                </div>
              )}
              {activeMessages.map((msg) => (
                ...
                >
                  <div
                    className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${msg.role === "user" ? "bg-slate-200" : "bg-indigo-100"}`}
                  >
                    {msg.role === "user" ? (
                  ...
                  <div
                    className={`max-w-[75%] rounded-2xl px-4 py-3 text-sm ${msg.role === "user" ? "bg-indigo-600 text-white" : "bg-white border border-slate-200 text-slate-800 shadow-sm"}`}
                  >
                    {msg.role === "user" ? (
                      msg.content
                    ) : (
            ...
            <div className="p-3 bg-white border-t border-slate-100">
              <form onSubmit={sendMessage} className="flex items-center gap-2">
                ...
                <input
                  ...
                  disabled={isLoading}
                  className="flex-1 px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 disabled:opacity-50"
                />
                <button
                  type="submit"
