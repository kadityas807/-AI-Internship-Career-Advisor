          ...
          {skills.map((skill, index) => (
            ...
            >
              ...
              <div className="absolute top-4 right-4 flex gap-2 opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 transition-opacity translate-y-2 group-hover:translate-y-0 group-focus-within:translate-y-0">
                <button onClick={() => openEditModal(skill)} aria-label={`Edit ${skill.name} skill`} className="p-2 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-xl transition-colors">
                  <Edit2 className="w-4 h-4" />
                </button>
                <button onClick={() => handleDelete(skill.id)} aria-label={`Delete ${skill.name} skill`} className="p-2 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-xl transition-colors">
