...
export default function SkillsPage() {
  ...
  const handleDelete = async (id: string) => {
    ...
    if (!window.confirm(`Are you sure you want to delete the skill "${skillToDelete?.name || 'this skill'}"?`)) return;
    try {
          ...
          {skills.map((skill, index) => (
            ...
            >
              ...
              <div className="absolute top-4 right-4 flex gap-2 opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 transition-opacity translate-y-2 group-hover:translate-y-0 group-focus-within:translate-y-0">
                ...
                <button onClick={() => handleDelete(skill.id)} aria-label={`Delete ${skill.name} skill`} className="p-2 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-xl transition-colors focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:outline-none">
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                <div>
                  <label htmlFor="skillName" className="block text-sm font-semibold text-slate-700 mb-2">Skill Name</label>
                  <input id="skillName" required type="text" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all outline-none" placeholder="e.g. React, Python, Public Speaking" />
                </div>
                <div>
                  <label htmlFor="skillCategory" className="block text-sm font-semibold text-slate-700 mb-2">Category</label>
                  <select id="skillCategory" value={formData.category} onChange={e => setFormData({...formData, category: e.target.value})} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all outline-none appearance-none">
                ...
                <div>
                  <label htmlFor="skillProficiency" className="block text-sm font-semibold text-slate-700 mb-2">Proficiency (1-5)</label>
                  <input id="skillProficiency" type="range" min="1" max="5" value={formData.proficiency} onChange={e => setFormData({...formData, proficiency: Number(e.target.value)})} className="w-full accent-indigo-600" />
                ...
                <div>
                  <label htmlFor="skillEvidence" className="block text-sm font-semibold text-slate-700 mb-2">Evidence (Optional)</label>
                  <textarea id="skillEvidence" value={formData.evidence} onChange={e => setFormData({...formData, evidence: e.target.value})} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all outline-none h-28 resize-none" placeholder="How did you learn or demonstrate this? e.g. Built a full-stack app, took a Coursera course." />
                </div>
