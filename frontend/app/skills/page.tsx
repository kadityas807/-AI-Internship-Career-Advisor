...
export default function SkillsPage() {
  ...
  const handleDelete = async (id: string, name: string) => {
    if (!user || !window.confirm(`Are you sure you want to delete the skill "${name}"?`)) return;
    try {
      ...
      >
        ...
        <div className="flex items-center gap-3">
          ...
          <motion.button
            ...
            className="flex items-center gap-2 px-5 py-2.5 bg-slate-900 text-white rounded-full font-medium hover:bg-slate-800 transition-colors shadow-sm shadow-slate-900/20 focus-visible:ring-2 focus-visible:ring-slate-900 focus-visible:ring-offset-2 focus-visible:outline-none"
          >
          ...
          {skills.map((skill, index) => (
            ...
            >
              ...
              <div className="absolute top-4 right-4 flex gap-2 opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 transition-opacity translate-y-2 group-hover:translate-y-0 group-focus-within:translate-y-0">
                <button onClick={() => openEditModal(skill)} aria-label={`Edit ${skill.name} skill`} className="p-2 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-xl transition-colors focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:outline-none">
                  <Edit2 className="w-4 h-4" />
                </button>
                <button onClick={() => handleDelete(skill.id, skill.name)} aria-label={`Delete ${skill.name} skill`} className="p-2 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-xl transition-colors focus-visible:ring-2 focus-visible:ring-red-500 focus-visible:outline-none">
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                <div>
                  <label htmlFor="skill-name" className="block text-sm font-semibold text-slate-700 mb-2">Skill Name</label>
                  <input id="skill-name" required type="text" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all outline-none" placeholder="e.g. React, Python, Public Speaking" />
                </div>
                <div>
                  <label htmlFor="skill-category" className="block text-sm font-semibold text-slate-700 mb-2">Category</label>
                  <select id="skill-category" value={formData.category} onChange={e => setFormData({...formData, category: e.target.value})} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all outline-none appearance-none">
                ...
                <div>
                  <label htmlFor="skill-proficiency" className="block text-sm font-semibold text-slate-700 mb-2">Proficiency (1-5)</label>
                  <input id="skill-proficiency" type="range" min="1" max="5" value={formData.proficiency} onChange={e => setFormData({...formData, proficiency: Number(e.target.value)})} className="w-full accent-indigo-600" />
                ...
                <div>
                  <label htmlFor="skill-evidence" className="block text-sm font-semibold text-slate-700 mb-2">Evidence (Optional)</label>
                  <textarea id="skill-evidence" value={formData.evidence} onChange={e => setFormData({...formData, evidence: e.target.value})} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all outline-none h-28 resize-none" placeholder="How did you learn or demonstrate this? e.g. Built a full-stack app, took a Coursera course." />
                </div>
                <div className="flex justify-end gap-3 mt-8 pt-4 border-t border-slate-100">
                  <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} type="button" onClick={() => setIsModalOpen(false)} className="px-5 py-2.5 text-slate-600 font-medium hover:bg-slate-100 rounded-xl transition-colors focus-visible:ring-2 focus-visible:ring-slate-200 focus-visible:outline-none">Cancel</motion.button>
                  <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} type="submit" className="px-5 py-2.5 bg-indigo-600 text-white font-medium rounded-xl hover:bg-indigo-700 transition-colors shadow-sm shadow-indigo-200 focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:outline-none">Save Skill</motion.button>
                </div>
              </form>
            </motion.div>
          </div>
