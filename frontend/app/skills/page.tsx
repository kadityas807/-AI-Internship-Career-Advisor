...
export default function SkillsPage() {
  ...
  const handleDelete = async (id: string, name: string) => {
    if (!user || !window.confirm(`Are you sure you want to delete the "${name}" skill?`)) return;
    try {
          ...
          {skills.map((skill, index) => (
            ...
            >
              ...
              <div className="absolute top-4 right-4 flex gap-2 opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 transition-opacity translate-y-2 group-hover:translate-y-0 group-focus-within:translate-y-0">
                ...
                <button onClick={() => handleDelete(skill.id, skill.name)} aria-label={`Delete ${skill.name} skill`} className="p-2 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-xl transition-colors focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:outline-none">
